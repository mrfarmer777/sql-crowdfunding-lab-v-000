...
"SELECT projects.title, SUM(pledges.amount)-projects.funding_goal FROM projects INNER JOIN pledges ON projects.id=pledges.project_id GROUP BY HAVING projects.funding_goal < SUM(pledges.amount)"
end
